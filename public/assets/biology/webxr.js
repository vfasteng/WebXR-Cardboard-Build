(function () {
  ...
  XRManager.prototype.onSelectStart = function (session) {
    this.hasVRTap = true;
  }

  XRManager.prototype.onSelectEnd = function (session) {
    this.hasVRTap = false;
  }

  XRManager.prototype.onSelect = function (session) {
    this.hasVRTap = true;
  }
  ...
  XRManager.prototype.getGamepads = function(frame, refSpace) {
    ...
    for (let source of frame.session.inputSources) {
      // TODO: Override specific gamepad data.
      if (true) {
        gamepads.push({
          id: 'test',
          index: 0,
          ...
          hasOrientation: true,
          hasPosition: true,
          hasVRTap: this.hasVRTap,
          orientation: this.GLQuaternionToUnity([0, 0, 0, 0]),
          position: this.GLVec3ToUnity([0, 0, 0]),
  ...
  XRManager.prototype.onSessionStarted = function (session) {
    ...
    // Handling input source information.
    var onSelectStartFunc = this.onSelectStart.bind(this);
    session.addEventListener('selectstart', onSelectStartFunc);
    var onSelectEndFunc = this.onSelectEnd.bind(this);
    session.addEventListener('selectend', onSelectEndFunc);
    var onSelectFunc = this.onSelect.bind(this);
    session.addEventListener('select', onSelectFunc);
